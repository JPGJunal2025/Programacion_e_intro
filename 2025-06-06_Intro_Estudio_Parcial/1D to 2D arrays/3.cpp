...
int main(int argc, char **argv)
{
  ...
  std::vector<double> array2d2(N*P, 0.0);
  fill_matrix(array2d2, N, P); // Llena con valores secuenciales
  std::cout << "Matriz con índices:\n";
  print_matrix(array2d2, N, P);

  std::vector<double> array2d_random2(N*P, 0.0);
  fill_matrix_random(array2d_random2, N, P, 123); // usa semilla fija
  std::cout << "Matriz aleatoria:\n";
  print_matrix(array2d_random2, N, P);
  
  std::vector<double> array2d_mult(M*P, 0.0);
  matmul(array2d_random1, array2d_random2, array2d_mult, M, N, P);
  std::cout << "Multiplicacion de Matriz:\n";
  print_matrix(array2d_mult, M, P);
